...
**Pulse** = macOS 菜单栏应用，AppUpdateChecker 工具。监听 macOS / Windows app 更新 + AI 榜单（v2.79.4+）。**多模态多数据源 Electron app**，主进程 Phase 3 + 3.5 完成；**Phase 4 renderer JSX→TSX 进行中**（Batch A：hooks/utils；Batch B：theme/diagnostics/watchlist/news/reminders/invest/digest/recent/wechat-hot/metals）。
...
- `src/renderer/` — 渲染进程（Preact + esbuild；**Phase 4**：hooks/utils + 小叶子域已 `.ts`/`.tsx`；components/store/大业务域/入口仍 `.js`/`.jsx`）
