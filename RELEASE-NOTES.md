...
- guest 保活改挂 `/im`（软刷新也 ping `/im`），让官方 IM 长连常在
- 已解析的 WS `chat` 帧直接弹通知（不依赖 `humanUnread` 上涨），再防抖 sync 对齐徽标
- seed 日志补 `unreadByType`，方便区分「真人未读=0 / 运营未读堆高」
- 设置文案说明：开「仅真人」时站点营销角标不会进 Pulse 徽标
...
- 优先在 guest 页内发 `session.sync`（与浏览器同 cookie）
- 认证失败改为最多 15 分钟一次软刷新，去掉狂 reload
- cookie 仍在时 UI 显示「消息同步异常」，不再逼「重新扫码」

## v3.3.2 (🐟 闲鱼通知体验增强) — 2026-09-14

**🐟 闲鱼消息 P0–P2 + 会话深链** — 协议层通知可用、可控、更贴近真 IM

- **通知摘要 + 会话深链**：未读上涨弹「昵称：最后一句」；点击尽量打开 `/im?peerUserId=&itemId=`，读后再 sync 对齐徽标
- **登录态可见**：页头「已登录 / 未登录 / 过期」pill + 一键重新登录；过期 toast 提示
- **设置可控**：桌面通知开关、仅提醒真人买家、立即检查；偏好持久化 `state.goofish`
- **近实时门铃**：guest 内钩官方 IM WebSocket → 防抖触发 `session.sync`；轮询降为 60s 兜底
- **免打扰 / 冷却**：复用全局 quiet hours；连续通知默认 ≥90s；退役 title/DOM 未读双写
...
**🧠 记忆自动沉淀**
- 关闭全局聊天时，若会话含「以后 / 我喜欢 / 别再…」等偏好，用 fast model 自动抽取 ≤3 条写入长期记忆
- 与已有记忆去重；fire-and-forget，不影响关闭手势
- 截图附加此前已支持（聊天工具栏 📎）

## v2.86.3 (📝 本机较新说明) — 2026-09-11

**📝 installed_newer badge 说明**
- 自更新 app（如 Marvis）本机版本会领先公开安装包
- badge 旁 hover 显示「本机版本领先公开安装包（多为应用内自更新）」
...
**🔧 macOS「退出并安装」进程退不干净**:
- 菜单栏应用点安装前先清 tray、去掉关窗 hide 拦截，并在 `before-quit-for-update` 时强制 `app.exit(0)`
- 避免更新已暂存却仍挂着进程，需手动退出再开才生效
