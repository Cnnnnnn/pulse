...
> - **strict 锁**：`tsconfig.app.strict.json`（`npm run typecheck` 已接入）。`any` 用 `ponytail:` 标 ceiling，禁 `@ts-ignore`
>
> 本 skill 教你怎么：
...
> 6. ~~Phase 3.5~~ 已完成；新 main `.ts` 直接满足 `tsconfig.app.strict.json`## 何时触发
