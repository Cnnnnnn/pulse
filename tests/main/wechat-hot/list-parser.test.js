...
const RAW_OK = {
  ...
  msg: "数据请求成功",
  data: [
    { index: 1, title: "甲酰胺 致癌", hot: "208万", url: "https://s.weibo.com/weibo?q=甲酰胺 致癌" },
    { index: 2, title: "宁德时代利润超7家车企总和", hot: "99万", url: "https://s.weibo.com/weibo?q=xxx" },
    { index: 3, title: "各地花式迎端午", url: "https://s.weibo.com/weibo?q=yyy" }, // 无 hot
...
describe("wechat-hot list-parser (xxapi weibo)", () => {
  it("parses successful payload, drops empty titles/urls, assigns rank by index", () => {
    ...
    expect(items[0]).toEqual({
      rank: 1, title: "甲酰胺 致癌", url: "https://s.weibo.com/weibo?q=甲酰胺 致癌", heat: "208万",
    });
  ...
  it("throws parse_failed on code != 200", () => {
    expect(() => parseWechatHotPayload({ code: 500, data: [] }))
      .toThrowError(expect.objectContaining({ reason: "parse_failed" }));
  });

  it("throws parse_failed on missing/non-array data", () => {
    expect(() => parseWechatHotPayload({ code: 200, data: null }))
      .toThrowError(expect.objectContaining({ reason: "parse_failed" }));
    expect(() => parseWechatHotPayload({ code: 200, data: "string-not-array" }))
      .toThrowError(expect.objectContaining({ reason: "parse_failed" }));
    expect(() => parseWechatHotPayload({ code: 200 }))
      .toThrowError(expect.objectContaining({ reason: "parse_failed" }));
  });
