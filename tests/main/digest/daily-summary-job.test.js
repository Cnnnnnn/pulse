...
describe("daily-summary-job", () => {
  ...
  beforeEach(() => {
    state = {
      daily_digest: { enabled: true, time: "08:30", last_push_date: null },
    };
    sentNotifications = [];
    deps = {
      getState: () => state,
      setState: (next) => {
        ...
        if (next.daily_digest)
          state.daily_digest = { ...state.daily_digest, ...next.daily_digest };
      },
      getConfig: () => ({
        notifications: { quiet_hours_start: "23:00", quiet_hours_end: "08:00" },
      }),
      sendNotification: (n) => sentNotifications.push(n),
      aggregate: () => ({
        date: "2026-06-20",
        sections: [{ kind: "updates", items: [{ name: "Cursor" }] }],
        lines: ["• Cursor 1 → 2"],
      }),
