  ...
  it("有 1 个更新: I7 summary + 检查更新段头 + 升级行 + 点击触发 onFocusUpdate", () => {
    ...
    const m = buildMenu({
      results: [
        {
          name: "Codex",
          installed_version: "26.609",
          latest_version: "26.611",
          has_update: true,
  ...
  it("全部最新 (无 update): I7 summary + 检查更新段头 + 提示行", () => {
    const m = buildMenu({
      results: [
        {
          name: "Cursor",
          installed_version: "3.7.42",
          latest_version: "3.7.42",
          has_update: false,
          status: "up_to_date",
          ts: Date.now(),
        },
...
describe("tray.buildMenu — 📊 AI 用量段 (Task B2)", () => {
  it('两 provider 都 unconfigured → 整段只显示"未配置"', () => {
    const m = buildMenu({
      results: [],
      aiUsage: {
        minimax: { status: "unconfigured" },
        glm: { status: "unconfigured" },
      },
    });
    // 找 "未配置" 行 (在 AI section 中)
    const unconfiguredRow = m.find(
      (i) => i.label && i.label.trim() === "未配置",
    );
  ...
  it("minimax ok + glm unconfigured → 显示 MiniMax 行 (含 percent + remainLabel),GLM 不显示", () => {
    ...
    const aiLines = m.filter(
      (i) =>
        i.label && (i.label.includes("MiniMax") || i.label.includes("GLM")),
    );
  ...
  it("aiUsage=null → 整段隐藏 (无 AI section 行)", () => {
    ...
    const aiLines = m.filter(
      (i) =>
        i.label &&
        (i.label.includes("MiniMax") ||
          i.label.includes("GLM") ||
          i.label.trim() === "未配置"),
    );
...
describe("tray.buildMenu — ⚽ 世界杯段 (Task C2)", () => {
  it("有今日比赛 → 显示 ⚽ 段 + 至少一行 team1 vs team2", () => {
    ...
    const matchRow = m.find(
      (i) =>
        i.label &&
        i.label.includes("Mexico") &&
        i.label.includes("South Africa"),
    );
  ...
  it("显示即将开始的下一场 (无今日比赛)", () => {
    ...
    const upcomingRow = m.find(
      (i) => i.label && i.label.includes("下一场") && i.label.includes("Spain"),
    );
...
describe("tray.buildMenu — ⚽ 世界杯段 clickable (Task C3)", () => {
  it("今日比赛行 → enabled: true (可点击)", () => {
    ...
    const m = buildMenu({
      ...
      worldcup: {
        todayMatches: [
          {
            key: "2026-06-17|13:00|Mexico|South Africa",
            team1: "Mexico",
            team2: "South Africa",
            time: "13:00",
          },
    ...
    expect(captured).toEqual({
      matchKey: "2026-06-17|13:00|Mexico|South Africa",
    });
  });
  ...
  it('"下一场" 行 → 也可点击 + 调 onFocusWorldcup', () => {
    ...
    const m = buildMenu({
      ...
      worldcup: {
        ...
        upcoming: [
          {
            key: "2026-06-18|15:00|Spain|France",
            team1: "Spain",
            team2: "France",
            time: "明天 15:00",
          },
  ...
  it("onFocusWorldcup 缺省 → 回调 stub 不抛异常", () => {
    ...
    const matchRow = m.find(
      (i) => i.label && i.label.includes("A") && i.label.includes("B"),
    );
...
describe("tray.buildMenu — 💎 贵金属段 (Task D1)", () => {
  it("有报价 → 显示 💎 段 + XAU 价格行 (USD/oz)", () => {
    ...
    const headerRow = m.find((i) => i.label && i.label.includes("💎"));
    expect(headerRow).toBeDefined();
    const priceRow = m.find(
      (i) => i.label && i.label.includes("XAU") && i.label.includes("3350"),
    );
  ...
  it("报价有变化 (price vs prevClose) → 行尾带 ↑/↓", () => {
    const m1 = buildMenu({
      ...
      metals: {
        quotes: {
          XAU: { price: 3400, prevClose: 3340, currency: "USD", unit: "oz" },
        },
        holdings: {},
        fetchedAt: Date.now(),
        errors: {},
    ...
    const m2 = buildMenu({
      ...
      metals: {
        quotes: {
          XAU: { price: 3300, prevClose: 3340, currency: "USD", unit: "oz" },
        },
        holdings: {},
        fetchedAt: Date.now(),
        errors: {},
