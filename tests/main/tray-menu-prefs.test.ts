...
describe("tray-menu-prefs: TRAY_SEGMENTS 单一真相", () => {
  it("包含 5 个 segment (v2.80 删 worldcup),key 集合为 updates/ai_usage/metals/check_action/config_action", () => {
    expect(TRAY_SEGMENTS).toHaveLength(5);
...
describe("tray-menu-prefs: DEFAULT_PREFS", () => {
  it("version=1,segments 包含 5 个 key 且全为 true", () => {
...
describe("tray-menu-prefs: normalizePrefs 纯函数", () => {
  it("只 updates:false → 其他 4 项仍 true", () => {
  ...
  it("value 不是 boolean → 补默认 true", () => {
    const r = normalizePrefs({
      segments: { updates: "false", ai_usage: null, metals: 0 },
    });
