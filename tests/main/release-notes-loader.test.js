...
describe('readReleaseNotes', () => {
  it('returns md content when file exists', () => {
    fs.writeFileSync(path.join(repoRoot, '.release-notes-2.32.0.md'), '# v2.32.0\n\n## 新增\n- foo');
  ...
  it('returns null on read error (does not throw)', () => {
    ...
    fs.mkdirSync(path.join(repoRoot, '.release-notes-2.32.0.md'));
...
describe('readSlides', () => {
  it('returns parsed slides when file exists and valid', () => {
    ...
    fs.writeFileSync(path.join(dir, 'slides.json'), JSON.stringify({
      version: '2.32.0',
      slides: [
  ...
  it('returns null on schema failure (missing version)', () => {
    const dir = path.join(contentRoot, '2.32.0');
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, 'slides.json'), JSON.stringify({ slides: [] }));
    expect(readSlides('2.32.0')).toBeNull();
  });
  ...
  it('returns null on schema failure (missing slides)', () => {
    const dir = path.join(contentRoot, '2.32.0');
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, 'slides.json'), JSON.stringify({ version: '2.32.0' }));
    expect(readSlides('2.32.0')).toBeNull();
  });
  ...
  it('returns null when slides array is empty', () => {
    const dir = path.join(contentRoot, '2.32.0');
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, 'slides.json'), JSON.stringify({ version: '2.32.0', slides: [] }));
    expect(readSlides('2.32.0')).toBeNull();
  });
});
