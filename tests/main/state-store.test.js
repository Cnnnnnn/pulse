...
// ─── 2026-06-14: App rollback · version_history ───

describe("getVersionHistory / saveVersionHistory (app rollback)", () => {
  it("文件不存在 → {}", () => {
    expect(getVersionHistory(statePath)).toEqual({});
  });

  it("老 state.json (无 version_history 字段) → {} (向后兼容)", () => {
    fs.writeFileSync(
      statePath,
      JSON.stringify({ v: 1, apps: {}, mutes: {} }),
      "utf-8",
  ...
  it("version_history 是数组 (损坏) → {}", () => {
    fs.writeFileSync(
      statePath,
      JSON.stringify({ v: 1, apps: {}, version_history: [] }),
      "utf-8",
    );
    expect(getVersionHistory(statePath)).toEqual({});
  });

  it("saveVersionHistory 写入 + getVersionHistory 回读 (round-trip)", () => {
    const map = {
      Cursor: [
        {
          from: "3.6.31",
    ...
    saveVersionHistory(map, statePath);
    expect(getVersionHistory(statePath)).toEqual(map);
  });

  it("落盘: state.json 顶层字段名是 version_history (snake_case)", () => {
    saveVersionHistory({ Cursor: [] }, statePath);
    const raw = JSON.parse(fs.readFileSync(statePath, "utf-8"));
    expect(raw).toHaveProperty("version_history");
    expect(raw.version_history).toEqual({ Cursor: [] });
  });

  it("saveVersionHistory 保留 apps / mutes / last_opened / active_category", () => {
    fs.writeFileSync(
      statePath,
      JSON.stringify({
        v: 1,
        apps: { Cursor: { name: "Cursor", latest_version: "3.6.32" } },
        mutes: { Cursor: { until: 0, reason: "manual" } },
        last_opened: { Cursor: { ms: 999, source: "spotlight" } },
        active_category: "dev",
      }),
    ...
    const raw = JSON.parse(fs.readFileSync(statePath, "utf-8"));
    expect(raw.apps.Cursor.latest_version).toBe("3.6.32");
    expect(raw.mutes.Cursor.until).toBe(0);
    expect(raw.last_opened.Cursor.ms).toBe(999);
    expect(raw.active_category).toBe("dev");
    expect(raw.version_history.Cursor[0].to).toBe("3.6.32");
  });

  it("多次写入 → 后写覆盖前写 (caller 自己管倒序/cap)", () => {
    saveVersionHistory(
      {
        Cursor: [
          { from: "3.6.30", to: "3.6.31", at: 1, backupPath: "/a", source: "brew", sizeBytes: 1 },
        ],
      },
      statePath,
    );
    saveVersionHistory(
      {
        Cursor: [
          { from: "3.6.31", to: "3.6.32", at: 2, backupPath: "/b", source: "brew", sizeBytes: 2 },
          { from: "3.6.30", to: "3.6.31", at: 1, backupPath: "/a", source: "brew", sizeBytes: 1 },
        ],
      },
      statePath,
    );
    const list = getVersionHistory(statePath).Cursor;
    expect(list).toHaveLength(2);
    expect(list[0].to).toBe("3.6.32");
    expect(list[1].to).toBe("3.6.31");
  });

  it("saveVersionHistory(null) 兜底为 {}, 不 throw", () => {
    saveVersionHistory(null, statePath);
    expect(getVersionHistory(statePath)).toEqual({});
  });
});
