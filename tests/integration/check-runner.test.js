...
function makePool(results = []) {
  return {
    enqueue: vi.fn(
      async () => results.shift() || { name: "X", status: "up_to_date" },
    ),
  };
}
...
function makeCtx({ poolResults = [], onCheckComplete, sendToRenderer } = {}) {
  ...
  const send = (channel, payload) => {
    ...
    else if (channel === "auto-check-finished")
      calls.autoFinished.push(payload);
    else if (channel === "check-progress") calls.progress.push(payload);
  };
  return {
    deps: {
      getConfig: () => ({
        apps: [{ name: "X", bundle: "X.app", detectors: [] }],
      }),
      pool: makePool(poolResults),
      getWindow: () => ({
        isDestroyed: () => false,
        webContents: { send: sendToRenderer || send },
      }),
...
describe("runCheck (Phase 16)", () => {
  ...
  describe("cooldown 抑制 (Phase 17 bug fix regression)", () => {
    ...
    it("cooldown 已过的 app 正常通知 (last_notified 超出窗口)", async () => {
      ...
      const state = {
        apps: {
          Cursor: { name: "Cursor", last_notified: now - 25 * 60 * 60 * 1000 },
        },
      };
      const deps = makeDeps({
        results,
