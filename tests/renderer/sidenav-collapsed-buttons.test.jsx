...
describe('SideNav 折叠态 — 顶部按钮可见性 (regression: 折叠后按钮被裁切)', () => {
  ...
  test('styles.css 中 .side-nav-header-actions 基础层 + 折叠层必须任一规则避免 40px 容器裁切', async () => {
    const fs = await import('node:fs');
    const path = await import('node:path');
    const css = fs.readFileSync(path.resolve('styles.css'), 'utf8');
    // 当前实现: 基础层用 nowrap (3 按钮横向优先) + 折叠层用 column (折叠后垂直堆叠),
    // 都能保证两个 28-32px 按钮都在 40px 容器内可见.
    const base = parseCssRule('.side-nav-header-actions', css);
    const collapsed = parseCssRule('.side-nav-collapsed .side-nav-header-actions', css);
    const baseOk =
      base['flex-wrap'] === 'wrap' || base['flex-wrap'] === 'nowrap'; // 基础层要么 wrap 要么保持 nowrap 让折叠层接管
    // 折叠层必须用 column 或 wrap 来实际换行
    const collapsedOk =
      collapsed['flex-direction'] === 'column' ||
      collapsed['flex-wrap'] === 'wrap';
    expect(baseOk && collapsedOk).toBe(true);
  });
