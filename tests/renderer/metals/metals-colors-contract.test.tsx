...
//   本文件锁住: tokens.css 含这两个 CSS 变量定义 + styles.css 两个 class 颜色映射到 --accent-red/green.
//
...
describe("metals 涨跌色视觉契约 (P6-audit-fix)", () => {
  it("tokens.css :root 定义 --metals-up / --metals-down", () => {
    // 中国股市惯例: 红涨绿跌, 跟 stocks.css 的 .stock-td.up/.stock-td.down 同源.
    expect(tokens).toMatch(/--metals-up:\s*var\(--accent-red\)/);
    expect(tokens).toMatch(/--metals-down:\s*var\(--accent-green\)/);
  });
  ...
  it("--color-success / --color-danger 已存在 (metals 变量依赖的 primitive)", () => {
    ...
    expect(tokens).toMatch(/--accent-green:/);
    expect(tokens).toMatch(/--accent-red:/);
  });
});