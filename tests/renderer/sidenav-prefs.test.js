...
describe("sidenav-prefs: reorderItems", () => {
  ...
  it("reorderItems: from → to 'before'", () => {
    const p0 = resetPrefs(); // [ithome, wechat-hot, worldcup, funds, metals, ai-usage, versions]
    const p1 = reorderItems(p0, "ithome", "funds", "before");
    expect(p1.order).toEqual([
      "wechat-hot", "worldcup", "ithome", "funds", "metals", "ai-usage", "versions",
    ]);
  });
  ...
  it("reorderItems: from → to 'after'", () => {
    ...
    expect(p1.order).toEqual([
      "wechat-hot", "worldcup", "funds", "ithome", "metals", "ai-usage", "versions",
    ]);
  });
  ...
  it("DEFAULTS_FOR_TESTS: 7 个 nav key", () => {
