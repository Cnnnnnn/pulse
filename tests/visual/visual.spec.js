...
/* ───────────────────────────────────────────────────────────
   ...
   真实运行时 minimax usage_summary 端点对订阅 key 返回 401, UI 必须降级
   到 4 分区骨架 + 公开 API windows 打包的 KPI. 验证浅/暗双主题下 fallback
   视觉无破绽.
   ─────────────────────────────────────────────────────────── */
...
const AI_USAGE_FALLBACK_FIXTURE = {
  ...
  // 关键: 故意不带 usageSummary, 触发 fallback 渲染
};

const pushFallbackFixture = `
  (function pushFallbackFixture() {
    if (typeof window.api === 'undefined') return;
    const origApi = window.api;
    const overrides = {};
    overrides.onAiUsageUpdated = function (cb) {
...
test("ai-usage tab — fallback (公开 API only) light baseline", async ({
  ...
  await page.addInitScript(pushFallbackFixture);
  await page.goto("/");
  await waitForShell(page);
  const aiTile = page.locator('[aria-label*="AI 用量"]').first();
  if (await aiTile.count()) {
  ...
  // ponytail: 等 .ai-usage-dashboard--fallback 出现 (不是普通 4 分区)
  await page.waitForSelector(".ai-usage-dashboard--fallback", { timeout: 15_000 });
