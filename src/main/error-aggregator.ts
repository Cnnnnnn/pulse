...
function makeId(ts: number, counter: number): string {
  const d = new Date(ts);
...
function deserializeSafe(line: string): any {
  try {
...
export function createAggregator(opts: any): any {
  ...
  function enqueueWrite(file: string, line: string): Promise<any> {
  ...
  async function append(input: any): Promise<any> {
  ...
  async function query(filter: any = {}): Promise<any> {
    const { since, limit, level } = filter;
    const cap = typeof limit === 'number' && limit > 0 ? limit : Infinity;

    let files;
    try {
      files = fs.readdirSync(logsDir).filter((f: string) => /^errors-\d{4}-\d{2}-\d{2}\.jsonl$/.test(f));
    } catch {
