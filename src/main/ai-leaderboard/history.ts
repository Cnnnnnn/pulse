/**
 * src/main/ai-leaderboard/history.ts
 *
...
/**
 * 获取最近一次历史 Arena 排名快照。
 * @param lookbackDays 最多往前扫几天
 * @returns {Map<string, object>} modelId → { [board]: { rank, score } }
 */
export function getPreviousArenaRanks(lookbackDays: number = 7): Map<string, any> {
 ...
 */
export function getArenaRankSeriesMap(nDays: number = 14): Map<string, Map<string, any[]>> {
  const today = new Date();
  const byDay: { date: string; rankMap: Map<string, any> }[] = []; // [{ date, rankMap: Map<id, {board:rank}> }]，时间升序
  ...
  for (const { date, rankMap } of byDay) {
    for (const [id, boardsObj] of rankMap) {
      ...
      for (const [board, info] of Object.entries(boardsObj as Record<string, any>)) {
 ...
 * 计算排名变动。
 * @param modelId
 * @param board Arena board name (text/vision/code)
 * @param currentRank 当前排名
 * @param prevRanks getPreviousArenaRanks() 的返回值
 * @returns {{delta: number|null, isNew: boolean}}
 *   delta > 0 表示上升（排名数字变小），< 0 表示下降，null 表示无历史数据
 */
export function computeRankDelta(modelId: string, board: string, currentRank: number, prevRanks: Map<string, any>): { delta: number | null; isNew: boolean } {
 ...
 * @param keepDays
 */
export function pruneOldCache(keepDays: number = 30) {
  ...
  try {
    ...
    for (const f of files) {
      ...
      const fp = path.join(dir as string, f);
      try {
