...
export async function getLeaderboard(opts: any = {}): Promise<any> {
  ...
  const need = (k: string) => sources[k] !== false;
  ...
  const hfModels = hfWrap.raw ? huggingfaceFetcher.normalize(hfWrap.raw) : [];
  ...
  return {
    ...
    attribution,
    count: shown.length,
    stale: arenaWrap.stale || aaWrap.stale || orWrap.stale || lbWrap.stale || mdWrap.stale || hfWrap.stale,
    fromCache:
      arenaWrap.fromCache &&
      aaWrap.fromCache &&
      orWrap.fromCache &&
