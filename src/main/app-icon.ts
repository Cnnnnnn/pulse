...
/**
 * @param bundlePath  e.g. '/Applications/Cursor.app'
 * @param deps        测试用 (fs / spawn / app / sipsPath)
 * @returns  base64 dataUrl 或 null
 */
 ...
 */
async function _loadIconUncached(bundlePath: string, deps: Deps): Promise<string | null> {
 ...
 */
export function findIcnsPath(bundlePath: string, deps: { fs: typeof fs }): string | null {
  // 1. Info.plist
  ...
  try {
    if (deps.fs.existsSync(resDir)) {
      ...
      const icns = entries.find((f: string) => f.toLowerCase().endsWith(".icns"));
 ...
 */
function convertIcnsToPngWithDeps(icnsPath: string, deps: ConvertDeps): Buffer | null {
  try {
    const out = path.join(
