...
function loadEnvGithubToken(): void {
...
export function getEnvGithubToken(): string {
  loadEnvGithubToken();
  return process.env.GITHUB_TOKEN || "";
}

/** 有 token 才返回 Authorization 头，避免污染未认证请求。导出供单测验证。 */
export function authHeader(token: any): { Authorization?: string } {
 ...
 */
export function parseRateLimitHeaders(headers: any): { rateLimitRemaining?: number; retryAfter?: number } {
  if (!headers) return {};
  const out: { rateLimitRemaining?: number; retryAfter?: number } = {};
 ...
 */
export async function fetchRepoMeta(owner: string, repo: string, token: string = ""): Promise<any> {
 ...
 */
export async function fetchReadmeRaw(owner: string, repo: string, branch: string, token: string = ""): Promise<string | null> {
  const candidates = [
    {
 ...
 */
export async function fetchRepoRelease(owner: string, repo: string, token: string = ""): Promise<any> {
  let res;
  try {
  ...
  const stripV = (t: string) => (typeof t === "string" ? t.replace(/^[vV]/, "") : "");
  const map = (r: any) => ({
 ...
 */
export async function fetchGithubProject(input: string, token: string = ""): Promise<any> {
