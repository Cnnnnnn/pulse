/**
 * src/main/diagnostics-aggregator.ts
 *
 ...
 */
export function computeTopFailures(entries: any[], topN: number = 5): any[] {
  if (!Array.isArray(entries) || entries.length === 0) return [];
  const buckets = new Map<string, any>(); // key → { source, message, count, firstTs, lastTs }
 ...
 * @returns {Buffer}
 */
export function buildTarHeader(name: string, size: number): Buffer {
 ...
 * @returns {Buffer}
 */
export function buildTar(parts: any[]): Buffer {
 ...
 * @returns {Buffer}
 */
export function buildTarGz(parts: any[]): Buffer {
 ...
 */
function collectErrorLogs(logsDir: string, sinceMs: number): any[] {
  if (!logsDir) return [];
  const parts: any[] = [];
  try {
    const files = fs.readdirSync(logsDir).filter((f: string) => /^errors-\d{4}-\d{2}-\d{2}\.jsonl$/.test(f));
 ...
 */
async function collectAggregated(aggregator: any, sinceMs: number): Promise<any[]> {
 ...
 * @returns {Array<object>}
 */
function collectRawLogs(logsDir: string, extraLogsDirs: string[], seenNames: Set<string>): any[] {
  const rawLogDirs: string[] = [];
 ...
 * @returns {Array<object>}
 */
function collectExtras(extras: any): any[] {
 ...
 */
function buildManifestPart(parts: any[], fileCount: number): any {
  const manifest = [
...
export async function bundleDiagnostics(opts: any): Promise<any> {
  const {
    logsDir,
    extraLogsDirs = [],
    aggregator,
  ...
  const parts: any[] = [];
  const seenNames = new Set<string>(); // for collectRawLogs 去重
