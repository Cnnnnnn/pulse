...
let quoteCache: { data: any; errors: any; fetchedAt: any } = { data: {}, errors: {}, fetchedAt: null };
let fxCache: { rate: number | null; fetchedAt: any } = { rate: null, fetchedAt: null };
 ...
 */
let backfillInflight: Promise<any> | null = null;

export type BackfillOpts = {
  httpGet?: (url: string, headers: any) => Promise<string>;
  now?: () => number;
  scheduler?: any;
  force?: boolean;
  skipInflightGate?: boolean;
};

export async function triggerBackfill(opts: BackfillOpts = {}): Promise<any> {
  ...
  const gate = opts.skipInflightGate
    ...
    : (backfillInflight = (async () => {
        try {
          ...
          try {
            const items = gap.need.map((n: any) => ({
              id: n.id,
              secid: n.secid,
            ...
            for (const [id, arr] of Object.entries(newHistory)) {
              const map = new Map();
              for (const p of [...(merged[id] || []), ...(arr as any[])]) {
                map.set(p.date, p);
              }
 ...
 */
export function getTraySnapshot(): any {
  return {
 ...
 */
export function registerMetalIpc(): void {
  ...
  ipcMain.handle("metals:history:get", () => {
    ...
    return {
      ...
      source: METALS.reduce((acc: any, m: any) => {
...
export type StartMetalSchedulerOpts = {
  onUpdateTray?: (snapshot: any) => void;
  getConfig?: () => any;
};
 ...
 */
export function startMetalScheduler(opts: StartMetalSchedulerOpts = {}): void {
  if (scheduler) return;
  const onUpdateTray =
...
export function stopMetalScheduler(): void {
  if (scheduler) {
    scheduler.stop();
    scheduler = null;
  }
