...
export function normalizePrefs(input: unknown): TrayPrefs {
  if (!input || typeof input !== "object" || !(input as any).segments) return DEFAULT_PREFS;
  const segs = (input as any).segments;
  if (typeof segs !== "object" || Array.isArray(segs)) return DEFAULT_PREFS;
  const out: TrayPrefs = { version: 1, segments: {} };
