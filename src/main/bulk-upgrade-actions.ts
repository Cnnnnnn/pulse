/**
 * src/main/bulk-upgrade-actions.ts
 *
 * Bulk Upgrade: 把 app item 映射成可执行 action.
 * 纯函数 — 给定 item 返回 action 描述, 真正执行交给 bulk-upgrade.ts.
 *
 * 字段来源 (per spec §8):
 ...
 */
export function getActionForApp(item: any): any {
 ...
 */
export function buildAppPath(bundleName: string): string {
