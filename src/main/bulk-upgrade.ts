 ...
 */
export async function runBulkUpgrade(opts: any): Promise<{
  succeeded: any[];
  failed: any[];
  skipped: any[];
  cancelled: boolean;
...
function makeError(message: string, output: string): Error {
 ...
 */
export async function defaultExec(action: any): Promise<any> {
  if (action.type === "brew") {
...
export function execBrew(cmd: string, args: string[]): Promise<any> {
  return new Promise((resolve, reject) => {
    childProcess.execFile(cmd, args, { timeout: 0 }, (err: any, stdout: string, stderr: string) => {
      ...
      if (err) {
        const e = new Error((stderr || err.message || "brew failed").trim()) as any;
        e.output = out;
        e.exitCode = err.code;
        reject(e);
        return;
...
async function execOpen(appPath: string): Promise<any> {
  // shell.openPath: 返回 string ('' = success, 非空 = error message)
  const result = await shell.openPath(appPath);
  if (result) {
    const e = new Error(`openPath failed: ${result}`) as any;
    e.output = result;
    throw e;
  }
...
async function execOpenUrl(url: string): Promise<any> {
  // shell.openExternal: 打开 URL (浏览器). 失败 → throw.
  if (!url || typeof url !== "string") {
    const e = new Error("open_url: missing url") as any;
    e.output = "";
    throw e;
  }
  try {
...
async function execMas(trackId: number, fallbackUrl: string): Promise<any> {
