...
function parseScorePair(raw: any): [number, number] | null {
 ...
 * @param line
 */
export function parseMatchLine(line: string): any {
...
function isGroupStageHeader(rest: string): boolean {
  return /^Group\s+[A-L]/i.test(rest);
}

function isKnockoutStageHeader(rest: string): boolean {
 ...
 * @param txt
 */
export function parseWorldcupTxt(txt: string): any {
  ...
  let currentWeekday: string | null = null; // 'Sun'..'Sat'
 ...
 * @param matches
 */
export function groupMatchesByDate(matches: any[]): any[] {
