/**
 * src/main/worldcup/bets-store.ts
 *
...
export function _validateInput(input: any): void {
  ...
  if (
    ...
    const err: any = new Error("invalid_stake");
  ...
  if (
    ...
    const err: any = new Error("invalid_pnl");
    err.code = "invalid_pnl";
    throw err;
  }
  if (typeof note !== "string" || note.length > MAX_NOTE_LEN) {
    const err: any = new Error("invalid_note");
...
export function upsert(input: any, statePath: any): any {
...
export function remove(date: any, statePath: any): any {
  if (typeof date !== "string" || !DATE_RE.test(date)) {
