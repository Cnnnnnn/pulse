...
const FIXTURES_URL = 'https://raw.githubusercontent.com/openfootball/worldcup/master/2026--usa/cup.txt';
 ...
 */
async function loadFixturesTxt() {
  // 1) Check cache
  try {
    const cached = stateStore.loadWorldcupTxt();
    if (cached && cached.txt && cached.ts && (Date.now() - cached.ts) < CACHE_TTL_MS) {
      return { ok: true, txt: cached.txt, cached: true, ts: cached.ts };
    }
  } catch (err) {
    mainLog.warn('[worldcup/fetcher] cache read failed', { msg: err && err.message });
  }
  ...
  try {
    ...
    try {
      ...
      mainLog.warn('[worldcup/fetcher] cache write failed', { msg: err && err.message });
    }
 ...
 * Renderer 应该调这个, 不直接 loadFixturesTxt.
 *
 * @returns {Promise<{ok: boolean, data?: {name, groups, matches}, cached?: boolean, ts?: number, reason?: string}>}
 */
async function fetchWorldcupFixtures() {
  const r = await loadFixturesTxt();
  if (!r.ok) return r;
  try {
    const data = parseWorldcupTxt(r.txt);
    return { ok: true, data, cached: r.cached, ts: r.ts };
  } catch (err) {
    mainLog.warn('[worldcup/fetcher] parse failed', { msg: err && err.message });
