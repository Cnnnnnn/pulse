...
const M_OF_WINNER = { A: 79, B: 85, D: 81, E: 79 - 0, G: 82, I: 77, K: 87, L: 80 };
...
//
// 源: FIFA WC26 Regs Annex C pp. 80-97, manganite/wm2026
// thirdPlaceAssignments.mjs (已验证 495 行覆盖全部 C(12,8) 组合).
const ANNEX_C_ROWS = [
...
// v1 默认 row (FIFA 2026 Annex C 行 1, 仅 6 个 best-third 入场, 对应晋级组
// {E,F,H,I,J,K,L} + 一个其他; 实际 FIFA Annex C row 1 = {A,B,C,D,E,F,I}+...).
// 真查表走 ANNEX_C_ROWS,这里仅作 fallback. R16+ 全部固定 (FIFA Article 12.6).
...
const R32_MATCH_OF_WINNER = { A: 79, B: 85, D: 81, E: 74, G: 82, I: 77, K: 87, L: 80 };

// Annex C lookup: key = row 内 8 字母按字母序 sort 后拼成的串 (C(12,8)=495 个 key, 覆盖全部).
// value = { rowIndex, rawLetters } 其中 rawLetters 按 ANNEX_C_WINNERS 列序.
 ...
 *
 * @param {string[]} advancingGroups - 8 个晋级第 3 名的组字母(未排序)
 * @returns {{rowIndex: number, config: object, matched: boolean, key: string|null}}
 */
function matchAnnexCCase(advancingGroups) {
  const cleaned = (advancingGroups || []).filter((x) => typeof x === 'string' && /^[A-L]$/.test(x));
 ...
 *
 * r32 slot 拆解规则:
 *   4 个固定 runner-up 互打: M73=A2 vs B2, M78=E2 vs I2, M83=K2 vs L2, M88=D2 vs G2
 *     (按 FIFA Article 12.6 fixed 配对, 不依赖晋级名单)
 *   8 个 winner vs best-3rd: M{ofWinner[X]} = winner 1X vs best-3rd <row[idx_of_X]>
 *     其中 idx_of_X 是 X 在 ANNEX_C_WINNERS 数组中的下标.
 *   剩下的 4 场: winner vs runner-up 配对 — FIFA Annex C fixed:
 *     M75=F1 vs C2, M76=C1 vs F2, M84=H1 vs J2, M86=J1 vs H2.
 *
 * 注: 上面的 runner-up 互打 / winner vs runner-up 配对是 FIFA Annex C 固定行
 * (与 best-3rd 晋级名单无关), 来自 Article 12.6.
 */
function buildAnnexConfigFromRow(rowIndex, rawLetters) {
  // rawLetters 是 8 个字母按 ANNEX_C_WINNERS 列序的 best-3rd 映射
  const bestThirdByWinner = {};
 ...
 *
 * v1.2: best-third 槽位现在真正挑队 — 从 pool 里取**第一个**仍在晋级
 * 名单 sortedThirdLetters 里的组 → 拿该组的 third.name 作 team.name.
 * (FIFA Annex C pool 通常只含 1 个候选组, v1.2 hardcode 真实 pool size.)
 *
 ...
 * @param {string[]} sortedThirdLetters - 已按 FIFA 排序的晋级 8 个组字母
 * @returns {{team: {name: string} | null, source: string, rank?: string, group?: string, pool?: string[]}}
 */
function resolveSlot(slotSpec, groupResults, sortedThirdLetters) {
  ...
  if (slotSpec.type === 'best-third') {
    ...
    const pick = (slotSpec.pool || []).find((g) => advancing.has(g));
    if (!pick) {
...
function computeBracket({ groupStandings, scores } = {}) {
  ...
  const sortedThirdLetters = sortedThird.map((s) => s.group);
