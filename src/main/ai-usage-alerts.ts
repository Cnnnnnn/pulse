...
export function normalizeAlertPrefs(raw: any): any {
  ...
  if (raw.lastNotified && typeof raw.lastNotified === "object") {
    for (const [pid, v] of Object.entries(raw.lastNotified as Record<string, any>)) {
...
export function detectOptsFromPrefs(prefs: any, lastNotifiedPercent: any): any {
  return {
...
export function topTasksByMsgCount(tasks: any, limit: number = 3): any[] {
  if (!Array.isArray(tasks)) return [];
  return tasks
    .filter((t: any) => t && typeof t.title === "string")
    .slice()
    .sort((a: any, b: any) => (b.msgCount || 0) - (a.msgCount || 0))
    .slice(0, limit);
}
 ...
 * @param args
 */
export function checkAiUsageAlertsPure({ providerId, historyDays, alertPrefs }: any): any {
