 ...
 * (require('../release-notes/loader.ts') + require('./state-store.ts')).
 */

const { createLogger } = require("./log.ts");
const defaultLoader = require("../release-notes/loader.ts");
 ...
 *
 * @param ctx
 * @param ctx.ipcMain   electron.ipcMain (prod 必传)
 * @param ctx.app       electron.app (prod 必传, 测试可省)
 * @param ctx.stateStore state-store 模块 (默认 default)
 * @param ctx.loader   release-notes loader 模块 (默认 default)
 */
export function registerReleaseNotes(ctx: any): void {
  if (!ctx || !ctx.ipcMain) {
  ...
  ipcMainRef.handle("release-notes:get-version", async (_evt: any, version: string) => {
  ...
  ipcMainRef.handle("release-notes:mark-seen", async (_evt: any, version: string) => {
    try {
