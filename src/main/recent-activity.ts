...
let _onUpdate: any = null; // push 后回调, 推 IPC 事件
let _cachedConfig: any = null; // config.json 缓存, 避免每次 push 都读盘
...
function _validateEntry(entry: any): { ok: boolean; reason: string } {
 ...
 * @returns {number}
 */
function _getMaxEntries(configPath: string, now: number): number {
...
let _defaultConfigPath: any = null;
function _setupDefaultConfigPath(fn: any): void {
  _defaultConfigPath = fn;
}

function _normalizeEntry(e: any): any {
...
function _normalizeAll(arr: any): any[] {
 ...
 */
function _dedupAndPush(entries: any[], entry: any, now: number, max: number): { entries: any[]; deduped: boolean } {
  const cap = Math.max(
    MIN_MAX_ENTRIES,
 ...
 */
export function push(entry: any, opts?: any): { ok: true; deduped: boolean } | { ok: false; reason: string } {
  const v: any = _validateEntry(entry);
  if (!v.ok) return v;
  const o = opts || {};
 ...
 * @returns {RecentActivityEntry[]}
 */
export function broadcast(opts?: any): any[] {
  const o = opts || {};
...
export function setOnUpdate(fn: any): void {
