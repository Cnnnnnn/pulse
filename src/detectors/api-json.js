...
function pickChangelog(obj) {
  ...
  return (
    ...
    obj.release_notes ||
    obj.changelog ||
    (obj.data && (obj.data.body || obj.data.releaseNotes || obj.data.changelog)) ||
    ''
  );
}
...
function pickVersion(obj) {
  ...
  // 1) 顶层常见字段
  const top = obj.version || obj.productVersion || obj.latest_version || obj.latestVersion
    || obj.appVersion || obj.app_version || obj.tag_name || obj.releaseName
    || obj.build_version || obj.buildVersion;
  if (top) return top;
  // 2) 嵌套: data.x  (WorkBuddy 部分 API 套了一层 { data: {...} })
  if (obj.data && typeof obj.data === 'object') {
    const nested = obj.data.version || obj.data.productVersion || obj.data.latest_version
      || obj.data.latestVersion || obj.data.appVersion || obj.data.app_version
      || obj.data.tag_name || obj.data.build_version;
    if (nested) return nested;
  }
  // 3) 嵌套: result.x / payload.x  (部分 API 习惯)
  for (const wrap of ['result', 'payload', 'response', 'body']) {
    if (obj[wrap] && typeof obj[wrap] === 'object') {
      const v = obj[wrap].version || obj[wrap].productVersion || obj[wrap].latest_version;
      if (v) return v;
    }
  }
  return null;
