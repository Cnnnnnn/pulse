...
export function buildKlineUrl(secid: any, beg: any, end: any) {
...
export function parseKlineResponse(text: any, secid: any) {
...
export function dedupeByDate(points: any, maxDays = 30) {
  const map = new Map();
...
export async function fetchMetalKline(items: any, httpGet: any) {
...
export function pointsToHistoryMap(fetched: any, items: any) {
  const out: any = {};
  for (const item of items) {
    ...
    // 多出的字段是纯加法.
    out[item.id] = fetched[item.id].map((p: any) => ({
      date: p.date,
      open: p.open,
      high: p.high,
      low: p.low,
...
export function isoDateOffset(dayOffset: any) {
  const d = new Date();
