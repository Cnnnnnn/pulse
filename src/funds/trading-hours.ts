 ...
 * }}
 */
export function getTradingStatus(now: any, holidays: any = []) {
 ...
 */
export function msUntilNextOpen(now: any, holidays: any = []) {
 ...
 */
export function msUntilNextFetch(now: any, { intervalMs, holidays = [] }: any) {
  const status = getTradingStatus(now, holidays);
