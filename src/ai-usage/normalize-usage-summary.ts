/**
 * src/ai-usage/normalize-usage-summary.ts
 *
...
export function normalizeUsageSummary(rawResponse, opts: any = {}) {
  ...
  const modelsTotals = (Object.values(modelTotals) as any[]).reduce((s: any, v: any) => s + v, 0);
  const modelBreakdown = (Object.entries(modelTotals) as any[])
    .map(([name, total]: any) => ({
      model: name,
      totalToken: total,
      sharePercent: modelsTotals > 0 ? Math.round((total / modelsTotals) * 1000) / 10 : 0,
    }))
    .sort((a: any, b: any) => b.totalToken - a.totalToken);
