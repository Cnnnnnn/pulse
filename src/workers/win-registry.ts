 ...
 */
export async function queryRegistryField(regPath, field, opts: any = {}) {
 ...
 */
export async function queryAllUninstallKeys(displayName, opts: any = {}) {
  ...
  for (const root of UNINSTALL_ROOTS) {
    try {
      ...
      for (const block of blocks) {
        const fields: any = parseRegOutput(block);
        if (
          fields.DisplayName &&
