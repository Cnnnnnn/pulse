...
import { getAngle } from "./stock-detail-angles";
 ...
 * }|null>}
 */
export async function fetchSingleAngle(httpClient: any, code: any, angleKey: any) {
  if (!code || !angleKey) return null;
  const angleDef = getAngle(angleKey);
  if (!angleDef) return null;
  const now = Date.now();
  try {
    const res: any = await angleDef.fetcher(httpClient, { code });
    if (res && res.ok) {
      const h = recordSuccess(code, angleKey);
      return {
        angleKey,
