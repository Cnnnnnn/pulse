...
function validateDetector(det, index, appName) {
  ...
  if (!isNonEmptyString(det.type)) {
    ...
    errs.push(`apps[${appName}].detectors[${index}].type: unknown '${det.type}'`);
  }
  if (det.timeout != null && (typeof det.timeout !== 'number' || det.timeout <= 0)) {
    errs.push(`apps[${appName}].detectors[${index}].timeout: must be positive number`);
  }
  if (det.url != null && !isNonEmptyString(det.url)) {
    errs.push(`apps[${appName}].detectors[${index}].url: must be non-empty string`);
  }
  if (det.cask != null && !isNonEmptyString(det.cask)) {
    errs.push(`apps[${appName}].detectors[${index}].cask: must be non-empty string`);
  }
  if (det.field != null && !isNonEmptyString(det.field)) {
    errs.push(`apps[${appName}].detectors[${index}].field: must be non-empty string`);
  }
  return errs;
}
...
function validateApp(app, index) {
  ...
  if (app.detectors == null) {
    ...
    errs.push(`apps[${label}].detectors: empty (at least one detector required)`);
  } else {
...
function validateConfig(input) {
  ...
  if (input.check_on_launch != null && typeof input.check_on_launch !== 'boolean') {
...
function sanitizeConfig(input) {
  ...
  for (const a of apps) {
    ...
    const cleanDets = dets.filter((d) =>
      isPlainObject(d) && isNonEmptyString(d.type) && VALID_DETECTOR_TYPES.has(d.type)
    );
    ...
    const cleanVS = vs
      .filter((s) => isPlainObject(s) && isNonEmptyString(s.type) && validVS.has(s.type))
      .map((s) => {
    ...
    cleanApps.push({
      ...
      release_notes_url: isNonEmptyString(a.release_notes_url) ? a.release_notes_url : undefined,
  ...
  const rawNotif = isPlainObject(input.notifications) ? input.notifications : {};
  ...
  if (typeof checkIntervalHours !== 'number' || !Number.isFinite(checkIntervalHours)) {
  ...
  const notifications = {
    quiet_hours_start: isNonEmptyString(rawNotif.quiet_hours_start) ? rawNotif.quiet_hours_start : null,
    quiet_hours_end:   isNonEmptyString(rawNotif.quiet_hours_end)   ? rawNotif.quiet_hours_end   : null,
    cooldown_hours:    (typeof rawNotif.cooldown_hours === 'number' && rawNotif.cooldown_hours >= 0)
      ? rawNotif.cooldown_hours : null,
