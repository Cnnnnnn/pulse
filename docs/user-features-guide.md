...
Pulse 是面向 macOS 与 Windows 的个人桌面信息中心。它以应用版本检查与升级为核心，同时整合新闻、世界杯、投资行情、AI 用量、GitHub 项目和 AI 榜单。
...
- [九、全局功能与快捷键](#九全局功能与快捷键)
- [十、设置与本地数据](#十设置与本地数据)
- [十一、故障排查与限制](#十一故障排查与限制)
...
- **侧边导航**：进入新闻、世界杯、投资、AI 用量、版本检查、GitHub 收录和 AI 榜单。
...
- 4 个 KPI：可升级应用数、自动可执行路径数、近 30 天升级成功数、近 30 天升级失败数。
- 明细表按应用展示上次升级结果（成功 / 失败 / 跳过）、时间、错误摘要。

每次批量升级或单点升级尝试都会自动记录到本地 `state.json`（按 app 滚动保留最近 200 条）。失败的应用可以一键重试，重试结果会覆盖「上次结果」列。
...
- **启用每日早报**：关闭后不再到点推送。
- **推送时间**：默认 `08:30`，支持 `HH:MM` 自定义。
- **免打扰起 / 止**：可选；命中区间则跳过推送。
- **模块订阅**：可升级应用 / 微博热搜 / IT 新闻 / 基金变动 / AI 用量，按需勾选。
- **LLM 改写**：默认关闭。开启后用大模型把要点改写成可读段落，失败时自动回退原要点。

#### 离线打开（v3.0）

Drawer 打开时优先尝试拉取实时数据；网络不通时自动回退到本地缓存的最近一次推送 snapshot，显示「离线模式」提示。

#### 导出 HTML 卡片（v3.0）

设置页提供「导出 HTML」按钮：

- 把当前或最近一次推送的早报渲染成单文件 HTML（内嵌 CSS，无外部依赖）。
- 导出完成后自动在 Finder / 资源管理器中定位文件。
- 适合通过邮件 / 消息直接转发给同事。
...
新闻、行情、比赛、版本、GitHub 和 AI 都可能访问外部服务。检查网络、代理和服务状态后重试；模块有最近成功缓存时会尽量继续展示旧数据。
