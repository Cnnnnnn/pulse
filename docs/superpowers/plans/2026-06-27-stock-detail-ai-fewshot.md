...
### Task 3: 修 advisor 拼 fewShot 到 system 段

**Files:**
- Modify: `src/ai/stock-detail-advisor.js:60` (在 system 拼接后追加 fewShot)
- Test: `tests/ai/stock-detail-advisor.test.js` (补 1 个 it 验证 fewShot 进入 system 段)

**Interfaces:**
- Consumes: `def.fewShot` (从 `resolvePrompt("stock_detail_analyze")` 返回)
- Produces: 修过的 `buildAnalyzeMessages`, system 消息含 `[system]\n\n[fewShot]` 形式

> ponytail: 原 plan 的"不动 advisor"基于一个错误假设 (advisor 已读 fewShot). reviewer 在 T2 末尾发现: `src/ai/stock-detail-advisor.js:60` 只拼 `def.system` + `def.rules`, 忽略 `def.fewShot`. 不修这个, T1/T2 的字段永远不到 LLM.

- [ ] **Step 1: 修 advisor.js line 60**
...
- [ ] **Step 2: 跑现有 advisor 测试, 确认无回归**

Run: `npx vitest run tests/ai/stock-detail-advisor.test.js`
Expected: 既有 N 个全 PASS (改动只是追加, 不改既有字符串).

- [ ] **Step 3: 加 1 个新 it 验证 fewShot 进入 system 段**

在 `tests/ai/stock-detail-advisor.test.js` 末尾追加:
...
Run: `npx vitest run tests/ai/stock-detail-advisor.test.js` → N+1 PASS
Run: `npx vitest run` → 全 PASS (T1 + 4 + 新 1 = 5, 加原 advisor 既有 = 总数 +1)
...
git commit -m "fix(ai): buildAnalyzeMessages 拼 fewShot 到 system 段"
```
