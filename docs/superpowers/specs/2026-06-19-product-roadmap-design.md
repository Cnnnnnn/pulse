...
| I2 | 可订阅 Watchlist(pin 任意 app / 基金 / 关键词,变化时通知)                      | 3    | 2    | 2    | 6    | 🟢 Next  | 🟤 立项中 (spec 待写) |
| I3 | SideNav 拖拽重排 + 隐藏                                                        | 2    | 1    | 0    | 7    | 🟢 Next  | 🟤 立项中 (spec 已写, 缺 plan) |
...
| I3 | SideNav 拖拽重排 + 隐藏 | 7 | 🟤 立项中(spec 已批 / plan 未写) | `docs/superpowers/specs/2026-06-22-i3-sidenav-drag-hide-design.md` 状态"设计已批准,待 writing-plans"。`src/renderer/components/SideNav.jsx` 仍是硬编码 `NAV_ITEMS`,无 `@dnd-kit / sortablejs / reorder` 依赖(全仓 grep 0 命中)。**卡点:缺 plan + 实施 commit,而不是 spec**(对照 §10.4 #1 偏差)。 |
...
| I6 | 内容标记已读(列表变灰 + SideNav badge) | 7 | ✅ 已落地 | ithome SideNav badge 联动 (ithome 已读本体此前已落地); `ithomeUnreadBadge` computed + SideNavItem badge prop + `.side-nav-badge` CSS; `tests/renderer/ithome-news-store.test.js` (3) + `sidenav-item-badge.test.jsx` (3) + `sidenav-ithome-badge.test.jsx` (3); wechat-hot badge 留 v2 |
...
| # | 候选 | spec | plan | 备注 |
| -- | --- | --- | --- | --- |
| I3 | SideNav 拖拽重排 + 隐藏 | ✅ `docs/superpowers/specs/2026-06-22-i3-sidenav-drag-hide-design.md`(2026-06-22 写) | ❌ | 卡在"spec → plan"流程中。**写作 §10.4 第 3 条偏差**的典型例子 |
| I2 | 可订阅 Watchlist | ❌ | ❌ | 2026-06-23 评估为下个切片候选 |
