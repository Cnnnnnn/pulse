...
| A3 | 全文搜索 Cmd+K(本地 inverted index,新闻 / 热搜 / AI digest)                    | 3    | 2    | 1    | 7    | 🟢 Next  | ⚫ 未立项 |
...
| C4 | 后台检测节流(智能时间窗) | 7 | ❌ 未开始 | `check-runner.js` 内无 `quietHours / offHours / smartWindow / throttle` 实现 |
...
| A3 | 全文搜索 Cmd+K(本地 inverted index) | 7 | ❌ 未开始 | 全仓无 `search-index / Cmd+K / inverted-index` 命中 |
...
| 功能 | 评分 | 状态 | 证据 |
|---|---|---|---|
| Twitter Serenity 面板(社交媒体信息源) | — | ✅ 已落地 | `src/main/twitter-serenity/`(10 文件:tweet-source + 3 sources + orchestrator + translator + cache-store + scheduler + manual-paste-parser + index);`src/renderer/twitter-serenity/`(6 文件:Panel + TweetList + TweetDetail + SourcesSettings + store + serenity-section);IPC `twitter:list/fetch/translate/sources:*/manual-paste`;state.json `twitterCache` + `twitterSources`(schema v2);`aggregate.js` serenity section;SideNav 第 8 项 `🐦`;spec `2026-06-22-twitter-serenity-panel-design.md` + plan `2026-06-22-twitter-serenity-panel.md`;18 task TDD 全绿(新增 ~70 case,全套 2370 passed) |
