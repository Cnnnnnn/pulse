...
| C5 | 增量 detector(近 7d 更新过的 app 跑全链,其余只跑第一个) | 2    | 2    | 1    | 6    | 🟢 已合入 (v2.42.0) | 🟢 已合入 |
| C6 | winget schema 适配 v1.6+                                 | 1    | 1    | 0    | 6    | 🟢 已合入 (v2.x 早, 详见 §14.2.1) | 🟢 已合入 |
| C7 | 检测结果导出(JSON / CSV)                                 | 1    | 1    | 0    | 6    | 🟢 已合入 (v2.35.0) | 🟢 已合入 |
...
| I7 | tray tooltip hover 显示完整摘要 5s (改: tray 菜单顶部总览)                     | 1    | 1    | 0    | 6    | 🟢 已合入 (v2.42.0) | 🟢 已合入 |
| I8 | 基金定投 / 盈亏提醒阈值                                                        | 2    | 1    | 1    | 7    | 🟢 已合入 (v2.37.0) | 🟢 已合入 |
...
| Q4 | startup time 目标化(< 800ms, v1 已合入)                                           | 2    | 2    | 1    | 6    | 🟢 已合入 v1 (v2.39-2.41), v2 待用户 | 🟢 已合入 |
...
| #  | 候选                                                                            | 价值 | 成本 | 风险 | 总分 | 状态     | 动工 |
| -- | ------------------------------------------------------------------------------- | ---- | ---- | ---- | ---- | -------- | ---- |
| A1 | changelog 摘要升级:多 detector / 多源交叉 + LLM 抽 "这版本最重要的 3 件事"       | 3    | 2    | 2    | 6    | 🟢 已合入 (v2.39.0) | 🟢 已合入 |
| A2 | "这版本该不该升"建议(基于 release notes + 使用频次,JSON schema 约束输出)       | 3    | 2    | 2    | 6    | 🟢 已合入 (v2.36.0) | 🟢 已合入 |
| A3 | 全文搜索 Cmd+K(本地 inverted index,新闻 / 热搜 / AI digest)                    | 3    | 2    | 1    | 7    | 🟢 Next  | 🟢 已合入 |
| A4 | AI 用量异常检测(7d sparkline + 异常告警 + 定位哪个 task)                        | 2    | 2    | 1    | 6    | 🟢 已合入 (v2.40-2.41) | 🟢 已合入 |
| A5 | 批量操作 AI 编排(升级前聚合 IT / 基金 / AI 用量生成"是否升级"理由)             | 2    | 2    | 1    | 6    | ⚪ Later | — |
| A6 | 本地小模型兜底(MLX / llama.cpp,默认关闭)                                        | 2    | 3    | 3    | 2    | ⚪ Later | — |
| A7 | AI 摘要 prompt 模板化(`config.json.aiDigest.prompts` 可改 + few-shot)           | 2    | 1    | 1    | 7    | 🟢 已合入 (v2.34 + v2.43 收尾) | 🟢 已合入 |
...
## 14. 四次对账(2026-06-24,v2.44.0 发版触发)

> 对账基线:`package.json` version = **2.44.0**(commit `08d699a`,已 push origin/main)。
> 本次对账由 v2.44.0(I6 v2 wechat-hot 角标清零)发版触发,跨度 v2.33 ~ v2.44 共 11 个版本。
> 重点:roadmap §3-6 概览表大量"未开始 / Maybe"项**实际已落地**,需要全面修正。

### 14.1 跨度概览(v2.33.0 → v2.44.0)

| 版本 | 切片 | 真实工作量 | 状态机观察 |
| --- | --- | --- | --- |
| v2.34.0 | A7 v2 收尾 | 69 行 | §10.6 标 🟡 / — , 实际合入 |
| v2.35.0 | C7 检测结果导出 | 200+ 行 | §3.1 标 ⚪ Later / ⚫ 未立项, 实际合入 |
| v2.36.0 | A2 升级建议 + A7 v2 Prompt 补全 | 400+ 行 | §6.1 标 A2 🟡 Maybe, 实际合入 |
| v2.37.0 | I8 基金盈亏阈值 | 中 | §4.1 标 I8 🟡 Maybe, 实际合入 |
| v2.38.0 | I2 Watchlist 扩展基金关键词 | 中 | (I2 已落地的迭代) |
| v2.39.0 | A1 Changelog 摘要 + Q4 v1 启动优化 | 800+ 行 | A1/Q4 标 🟡 Maybe, 实际合入 |
| v2.40.0 | I2 + Q4 + A4 | 中 | 多项延续 |
| v2.41.0 | Q4 v1(收尾) + A4 阈值可配置 + I6 SideNav 多源 | 1000+ 行 | 完整切片 |
| v2.42.0 | C5 增量 detector + I7 tray 摘要 | 698 行 | C5 标 ⚫ 未立项, I7 标 ⚪ Later, 实际合入 |
| v2.43.0 | A7 v3 daily digest LLM 改写 | 1041 行(含残余) | A7 基建已落 100%, 实际只补最后一公里 |
| v2.44.0 | I6 v2 wechat-hot 角标清零 | 516 行(含残余) | I6 基建已落 100%, 实际只补 1 个 hook |

### 14.2 本次对账核心发现:roadmap 严重低估"已落地"面

下面列出来,每条都有 git commit 证据。

#### 14.2.1 实际已合入,roadmap 仍标"未开始"的项

| 候选 | 评分 | roadmap 状态 (错误) | 实际状态 | 落地证据 |
| --- | --- | --- | --- | --- |
| **C5** 增量 detector | 6 | 🟡 Maybe / ⚫ 未立项 | ✅ v2.42.0 合入 | commit `5678ca0` |
| **C6** winget schema 适配 v1.6+ | 6 | 🟡 Maybe / ⚫ 未立项 | ✅ winget detector 在 v2.x 早已存在(具体版本待对账) | `src/detectors/winget-show.js` |
| **C7** 检测结果导出 (JSON/CSV) | 6 | ⚪ Later / ⚫ 未立项 | ✅ v2.35.0 合入 | commit `addf152` |
| **A1** changelog 摘要升级 | 6 | 🟡 Maybe / — | ✅ v2.39.0 合入 | commit `d6491e0` (`src/ai/changelog-summary.js`) |
| **A2** "这版本该不该升"建议 | 6 | 🟡 Maybe / — | ✅ v2.36.0 合入 | commit `cd0d715` (`src/ai/upgrade-advice.js`) |
| **A4** AI 用量异常检测 | 6 | 🟡 Maybe / — | ✅ v2.40-2.41 合入(异常检测 + 阈值可配置) | commit `9de3bd8` / `bc0575f` |
| **A7** AI 摘要 prompt 模板化 | 7 | 🟢 Next / 🔵 实施中 | ✅ v2.34.0 合入 + v2.43 收尾 | commits `4425ee8` / `63a6ac7` |
| **I7** tray tooltip hover 摘要 | 6 | ⚪ Later / — | ✅ v2.42.0 改为"tray 菜单顶部总览"实现(macOS 无 hover API) | commit `5678ca0` |
| **I8** 基金定投 / 盈亏提醒阈值 | 7 | 🟡 Maybe / — | ✅ v2.37.0 合入 | commit `f385f10` |
| **Q4** startup time 目标化 | 6 | 🟡 Maybe / — | ✅ v2.39-2.41(v1 落地) + v2 v2.41 (LazyNavPanel ESM splitting) | commits `d6491e0` / `bc0575f` |

#### 14.2.2 实际真未做

| 候选 | 评分 | roadmap 状态 | 评估 |
| --- | --- | --- | --- |
| **I4** 聚合源可插拔接口 | 6 | 🟡 Maybe / — | 真未做。YAGNI:目前 4 个聚合源(hot/news/funds/ai-usage/worldcup)无 3+ 源共性抽取需求 |
| **Q2** 首次启动 Welcome Wizard | 6 | 🟡 Maybe / — | 真未做。已被 v2.31.1 "release notes 首启向导" 半覆盖,优先度低 |
| **Q3** Windows 系统集成拉齐 macOS | 6 | 🟡 Maybe / — | 真未做。Windows 用户基数小,优先度低 |
| **Q9** 依赖升级(Electron 35 / Preact 10.22) | 4 | ⚪ Later / — | 真未做。优先度低,破坏性大 |
| **A5** 批量 AI 编排 | 6 | ⚪ Later / — | 真未做。依赖 A1/A2 稳定,roadmap 评分无优势 |
| **A6** 本地小模型兜底 | 2 | ⚪ Later / — | 真未做。评分 2,基本否决 |

### 14.3 总览刷新(v2.44.0 基线)

| Pillar | Next 总数 | ✅ 已落地 | 🟡 部分落地 | ❌ 真未开始 | ⚪ Later/不推荐 |
| --- | --- | --- | --- | --- | --- |
| 1 核心升级 | 7 (C1-C7) | **7** (含 C5/C6/C7 修正) | 0 | 0 | 0 |
| 2 信息聚合 | 8 (I1-I8) | **8** (含 I7 改为 tray 菜单摘要 / I8 已落) | 0 | 0 | 0 (I4 实际仅 Maybe, 见 §14.2.2) |
| 3 平台质量 | 9 (Q1-Q9) | 5 (Q1/Q5/Q6/Q7/Q8) | 0 | 0 | 4 (Q2/Q3/Q4v2/Q9) |
| 4 AI 驱动 | 7 (A1-A7) | **7** (含 A1/A2/A4/A7 修正) | 0 | 0 | 0 (A5/A6 评分低) |
| **合计** | **31** | **27** | **0** | **0** | **4** |

> **重大变化**:roadmap 主体(Next + Maybe)已 **100% 落地**!剩余工作全部来自"Later / Q4 v2 升级 / 依赖升级" 等本身优先度低或需要外部输入的项。

### 14.4 评分复盘 — 真实成本远低于 spec 估算

最近 3 个 sprint(v2.42 / v2.43 / v2.44)做了一个**关键实验**:每次动手前先**调研现状**,结果发现:

| Sprint | spec 估算工作量 | 实际工作量 | 实际 / 估算 |
| --- | --- | --- | --- |
| **C5 + I7** (v2.42) | C5: 增量 detector 需新建 detector 调度框架 + I7: tray hover tooltip 需查 Electron API | C5: ~80 行(只加 `decideIncremental` 纯函数 + inject) + I7: ~40 行(顶部 summary 行) | **~20%** |
| **A7 v3** (v2.43) | "需从零做 daily digest ↔ LLM 接入 + 配置化 + Settings 面板" | prompt-registry / state-store / IPC / PromptSettings 已在 v2.34-2.36 全落地, 实际只加 `tryRewriteSummary` + 一个新 prompt key | **~15%** |
| **I6 v2** (v2.44) | "wechat-hot 从零补 read 概念" | read-store / IPC / signal / SideNav 集成全已在 v2.32 落地, 实际只加 `clearWechatHotUnreadBadge` + `setActiveNav` 1 行 hook | **~5%** |

**根本原因**:
- 项目经过 v2.25 ~ v2.40 长跑沉淀,**基建覆盖面远超 roadmap 维护者认知**
- roadmap 状态机(🟢 🟡 ⚪)与"已落地"是**两套正交维度** — 状态机反映"评分 + 意向","已落地"反映 git log
- §2.4 "动工状态"列加了,但**从未随每次发版更新**

### 14.5 评分复盘 — spec 的"成本"维度长期被高估

原评分口径 `成本 (反向): 实现 + 测试 + 文档的人天. 0 = 巨大, 3 = 一天内`。
近期项目实际"一天内能合入"的项目数:
- v2.41 Q4 + A4 + I6: 1000+ 行, 3 个独立切片
- v2.42 C5 + I7: 698 行, 2 个独立切片
- v2.43 A7 v3: 1041 行(主要被"格式引号噪音"夸大,真实功能 ~150 行)

**结论**:**"成本 = 1"(1 天内)这个评分在当前阶段基本人人可达**,评分口径应改为:
- 0 = 跨多文件, 需新依赖 / 破坏性 schema 变更 (>1000 行)
- 1 = 2-5 个文件, 无新依赖, 主功能 + 测试 1 天可合入 (当前多数项目在此)
- 2 = 单文件小改 (e.g. i7 v2 wechat-hot 角标清零 1 行 hook)
- 3 = 1-2 行代码 (e.g. i18n key 修正)

否则"成本"分没有区分度。

### 14.6 真正的"未做"项目 — 全剩 Q/A 类

剔除掉"评分低 / 优化性 / 外部依赖"后,真正可能值得做的项目只剩:
1. **Q4 v2** — DevTools profile 数据收集脚本(被外部数据阻塞,等用户)
2. **A1→A2 升级建议完善** — A1/A2 已落,继续打磨 UI / schema / prompt,评分 6
3. **依赖升级 Q9** — Electron 35 / Preact 10.22 升级,破坏性大,优先度低

roadmap §3-6 列出的"未做"候选基本清零。下一次"发版触发对账"时,应该把"主体已 100% 落地"这一事实写进 §3-6 总览表,不再保留过时状态。

### 14.7 文档维护机制建议(给作者本人)

§13.5 提出"git hook 检测 release-notes 变更 → 提醒对账",本次对账确认**手动对账够用**,但成本高(对账 §14 用了 1 个工作 sprint 1/3 的时间)。

**建议改为**:
- 每个 release 强制 commit 一个 `.release-notes-X.Y.Z.md` + **自动**给"发版时未对账项"加 ⏳ 标记
- 半年 / 季度做一次完整对账(像本 §14)
- **roadmap 文档不再逐条维护"动工"列**,只维护"已落地"列 — 后者就是 git log
- "意向"维度保留在评分表即可

### 14.8 建议下一步(供选择,不替作者决策)

按"评分 + 不被外部依赖阻塞 + 项目重心"排序:

1. **A1 / A2 持续打磨** — 评分 6,但 A1/A2 是项目差异化的核心(LLM 升级建议), 值得加 better prompt + 更多 case 覆盖
2. **Q4 v2** — 等用户提供 DevTools profile 数据, 即可做"目标 < 800ms"的对齐脚本
3. **roadmap 大改版** — 既然主体 100% 落地, 应该写一份 **v2 路线图** 替代本 v1 (4 Pillar 已闭合), 新方向:多端 (iOS / Web) / 团队协作 / 商业化路径

**不建议现在做**:I4 / Q2 / Q3 / Q9 / A5 / A6(评分低 / 优先度低);新 Pillar 5+ 立项(在没有商业 / 团队需求下,新 Pillar 缺乏驱动)。
