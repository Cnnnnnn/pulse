...
```text
                          ┌─────────────────────────────────────────┐
                          │  路径分叉 (在 [browsing] → 关闭 时):      │
                          │   • 入口 = auto-open → 调 mark-seen     │
                          │   • 入口 = click 📖 → 不调 mark-seen    │
                          └─────────────────────────────────────────┘
...
[browsing]                 ← 显示当前 slide, prev/next 可点
   │ user 跳过/完成/ESC/点遮罩
   │ (按入口决定是否 IPC: mark-seen)
   │ resolve
   ▼
[hidden]                   ← 红点消失 (仅 auto-open 路径; manual 路径红点状态不变)
```

`browsingStore.entryPath` (在 §3.5 `release-notes-store.js` signals 之一) 在 open 时记录 (`'auto'` 或 `'manual'`); 关闭时按 entryPath 决定是否调 mark-seen。这是唯一一处状态机分叉, 其他行为两种入口完全一致。
